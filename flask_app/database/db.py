...
class AvisoAdopcion(Base):
    ...
    # Solo agregamos comentarios a avisos
...
#agregamos la tabla para comentario
...
def get_comentario_by_aviso(aviso_id):
    session = SessionLocal()
...
def insert_comentario(aviso_id, nombre, texto):    
    ...
    try:
        com = Comentario(aviso_id = aviso_id, nombre = nombre, texto = texto, fecha = datetime.now())
        session.add(com)
        session.commit()
        return com.id
    finally:
...
#una sin distinguir entre perros y gatos, y otra distinguiendolos
...
def get_stats_por_mes_tipo(year=None):

    session = SessionLocal()
    try:
        if year is None:
            year = datetime.now().year
        # cuenta por mes y tipo
        rows = (
            session.query(
        ...
        # armar dict mes {gato, perro}
        agg = {}
        for m, t, c in rows:
            m = int(m)
            agg.setdefault(m, {"mes": m, "gato": 0, "perro": 0})
            agg[m][t] = int(c)
        # asegurar meses faltantes en 0
        result = []
        for m in range(1, 13):
            result.append(agg.get(m, {"mes": m, "gato": 0, "perro": 0}))
        return result
    finally:
        session.close()
